...
import android.view.KeyEvent; // <--- NEW IMPORT FOR D-PAD / MEDIA KEYS
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // --- MODIFIED: Setup includes TV D-pad seeking logic now ---
    ...
    /**
     * Attaches a touch listener (for mobile) and a key listener (for Android TV/Firestick)
     * to the custom progress bar to enable seeking.
     * * THIS METHOD WAS MODIFIED TO ADD ONKEYLISTENER FOR D-PAD SEEKING.
     */
    private void setupProgressBarSeeking() {
        ...
        progressBar.setOnTouchListener((v, event) -> {
            ...
            // Standard touch seek logic remains the same
        ...
        progressBar.setOnKeyListener((v, keyCode, event) -> {
            ...
            switch (keyCode) {
                ...
                case KeyEvent.KEYCODE_DPAD_RIGHT:
                    ...
                    break;
                default:
                    return false; // Not a seek key, let the system handle it (e.g., D-pad Up/Down for focus)
            }
    ...
    private void showPlaybackSpeedDialog() {
        // CORRECTED SPEED LIST AS PER USER REQUEST
    ...
    // NEW METHOD: Handles D-pad and media key events for Android TV/Firestick
    @Override
    public boolean dispatchKeyEvent(KeyEvent event) {
        // 1. D-Pad Center (Select) Key handling for controls visibility
        if (event.getAction() == KeyEvent.ACTION_DOWN && 
            event.getKeyCode() == KeyEvent.KEYCODE_DPAD_CENTER) {
            
            // If the custom controls are hidden (meaning nothing is focused on screen)
            if (customControls.getVisibility() != View.VISIBLE) {
                toggleControls(); // Show controls and reset the timeout
                return true; // Consume the key event
            }
            // If controls are visible, allow the event to fall through to the focused view (buttons)
        }

        // 2. Play/Pause Key (Standalone media key on some remotes)
        if (event.getAction() == KeyEvent.ACTION_DOWN && 
            ...
            if (player != null) {
                ...
                updatePlayPauseButton();
                // Show controls briefly on media key press
                customControls.setVisibility(View.VISIBLE); 
                resetControlsTimeout();
                return true;
            }
        ...
        // 3. Delegate all key events (including D-pad Left/Right for seeking) to the PlayerView.
        // This allows ExoPlayer's built-in control logic to handle seeking/FF/RW when the 
        // custom controls overlay is NOT visible AND we are not on the custom progress bar.
        if (playerView != null) {
            // Note: If a custom view (like the progress bar) is focused, its OnKeyListener
            // will execute before this delegation, and if it returns true, this won't run.
            if (playerView.dispatchKeyEvent(event)) {
                // If the PlayerView successfully handled a seek/FF/RW, show controls briefly.
                customControls.setVisibility(View.VISIBLE);
                resetControlsTimeout();
                return true;
            }
        ...
        // 4. Fallback to default system behavior for all unhandled keys.
